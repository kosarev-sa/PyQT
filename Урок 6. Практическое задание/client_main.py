...
def arg_parser():
    ...
    if not 1023 < server_port < 65536:
        log.critical(f'Ошибка номера порта: {server_port}. Допустимы номера с 1024 до 65535')
        exit(1)
...
if __name__ == '__main__':
    ...
    if not client_name or not client_passwd:
        ...
        if start_dialog.ok_pressed:
            ...
            exit(0)
    log.info(f'Запущен клиент. Сервер: {server_address}, порт: {server_port}, имя пользователя: {client_name}')
