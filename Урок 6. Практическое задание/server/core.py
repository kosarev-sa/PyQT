...
from globals.descriptors_classes import PortValidator, AddressValidator
from globals.metaclasses_verifiers import ServerVerifier
from globals.variables import MAX_CONNECTIONS, DESTINATION, SENDER, ACTION, ACCOUNT_NAME, USER, RESPONSE_200, \
    RESPONSE_400, ERROR, MESSAGE, MESSAGE_TEXT, TIME, EXIT, GET_CONTACTS, RESPONSE_202, LIST_INFO, ADD_CONTACT, \
    CONTACT, REMOVE_CONTACT, USERS_REQUEST, PRESENCE, RESPONSE, PUBLIC_KEY, DATA, RESPONSE_205, RESPONSE_511, \
    PUBLIC_KEY_REQUEST
from globals.utils import send_message, get_message
from globals.log_decors import LogDecCls, login_required
...
class Server(threading.Thread):
    ...
    def run(self):
        ...
        while self.running:
            ...
            try:
                if self.clients:
                    recv_data_lst, self.listen_sockets, self.error_sockets = select(self.clients, self.clients, [], 0)
    ...
    def init_socket(self):
        log.info(
            f'Запущен сервер, порт: {self.port} , адрес с которого принимаются подключения: {self.addr}. '
    ...
    def process_message(self, message):
        if message[DESTINATION] in self.names and self.names[message[DESTINATION]
            ...
            log.error(f'Связь с клиентом {message[DESTINATION]} потеряна. Соединение закрыто, доставка невозможна')
            self.remove_client(self.names[message[DESTINATION]])
        else:
            log.error(f'Пользователь {message[DESTINATION]} не зарегистрирован, отправка сообщения невозможна')
