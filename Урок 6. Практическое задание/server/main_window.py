...
class MainWindow(QMainWindow):
    def __init__(self, database, server, config):
        ...
        self.show_history_button = QAction('Статистика сообщений', self)
        # Статусбар
        self.statusBar()
        ...
        self.label = QLabel('Список клиентов online:', self)
    ...
    def create_users_model(self):
        ...
        lst_users = self.db.active_users_lst()
        list = QStandardItemModel()
        list.setHorizontalHeaderLabels(
            ['Клиент', 'IP', 'Порт', 'Время подключения'])
        for row in lst_users:
