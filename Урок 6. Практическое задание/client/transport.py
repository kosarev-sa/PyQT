...
from globals.variables import ACTION, PRESENCE, TIME, USER, ACCOUNT_NAME, RESPONSE, ERROR, MESSAGE, SENDER, \
    DESTINATION, MESSAGE_TEXT, GET_CONTACTS, LIST_INFO, USERS_REQUEST, ADD_CONTACT, CONTACT, REMOVE_CONTACT, EXIT, \
    PUBLIC_KEY, DATA, RESPONSE_511, PUBLIC_KEY_REQUEST
...
class ClientTransport(threading.Thread, QObject):
    ...
    def connection_init(self, port, ip):
        ...
        log.debug('Запускаем процедуру авторизации')
        ...
        with socket_lock:
            ...
            try:
                ...
                log.debug(f'Ошибка соединения при авторизации', exc_info=err)
    ...
    def run(self):
        ...
        while self.running:
        # Ждём секунду и пробуем захватить сокет. Если не задержаться, отправка может долго ждать освобождения сокета
            time.sleep(1)
            message = None
            with socket_lock:
                try:
