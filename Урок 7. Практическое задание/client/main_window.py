...
from globals.variables import MESSAGE_TEXT, SENDER
...
class ClientMainWindow(QMainWindow):
    '''Класс - основное окно пользователя. Содержит всю основную логику
    работы клиентского модуля. Конфигурация окна создана в QTDesigner и
    загружается из конвертированого файла main_window_conv.py'''
    def __init__(self, database, transport, keys):
        super().__init__()

        self.db = database
        self.s = transport
        # объект - дешифорвщик сообщений с предзагруженным ключом
    ...
    def set_disabled_input(self):
        ...
        self.ui.label_new_message.setText(
            'Выберите получателя в окне контактов двойным кликом мыши.')
    ...
    def history_list_update(self):
        ...
        lst = sorted(self.db.get_msg_history(
            self.current_chat), key=lambda item: item[3])
        ...
        # и исходящие, выделяем их разным фоном.
    ...
    def set_active_user(self):
        ...
        self.ui.label_new_message.setText(
            f'Ввод сообщенния для {self.current_chat}:')
    ...
    def send_message(self):
        ...
        if not msg_text:
            return
        try:
            self.s.send_message(self.current_chat, msg_text)
            pass
        except ServerError as err:
