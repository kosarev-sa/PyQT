...
class ClientDbAlchemy:
    ...
    def __init__(self, name):
        # Разрешено несколько клиентов одновременно, каждый должен иметь БД
        # Клиент мультипоточный, надо отключить проверки на подключения с
    ...
    def add_known_users(self, users_lst):
        # Пользователи получаются с сервера, поэтому таблица очищается
    ...
    def get_known_users(self):
        return [user[0]
                for user in self.session.query(self.KnownUsers.name).all()]
...
if __name__ == '__main__':  # Отладка
    ...
    test_db.save_message(
        'test1',
        'test2',
        f'Тестовое сообщение 1. Время: {datetime.now()}')
    test_db.save_message(
        'test2',
        'test1',
        f'Тестовое сообщение 2. Время: {datetime.now()}!')
