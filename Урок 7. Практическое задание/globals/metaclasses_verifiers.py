...
class ClientVerifier(type):
    ...
    def __init__(self, clsname, bases, clsdct):
        ...
        # get_message или send_message из globals - корректное использование
        # сокетов
        if not ('get_message' in methods or 'send_message' in methods):
            raise TypeError('Функции, работающие с сокетами не используются!')
        # В случае если сокет не инициализировался для работы по TCP (аргументы
        # SOCK_STREAM, AF_INET), исключение:
...
class ServerVerifier(type):
    ...
    def __init__(self, clsname, bases, clsdct):
        ...
        if 'connect' in methods:
            raise TypeError(
                'Метод "Сonnect" недопустим в серверной пррограмме')
        # В случае если сокет не инициализировался для работы по TCP (аргументы
        # SOCK_STREAM, AF_INET), исключение:
        if not ('SOCK_STREAM' in attrs and 'AF_INET' in attrs):
            raise TypeError('Некорректная инициализация сокета!')
