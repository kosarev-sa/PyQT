...
class AddContactDialog(QDialog):    # Диалог выбора контакта для добавления
    def __init__(self, transport, database):
        ...
        self.btn_refresh.clicked.connect(self.update_possible_contacts)     # Назначаем действие на кнопку обновить
    ...
    def possible_contacts_update(self):
        ...
        # Удалим себя из списка пользователей, чтобы нельзя было добавить самого себя
        users_lst.remove(self.s.username)
        # Добавляем список возможных контактов
        self.selector.addItems(users_lst - contacts_lst)

    # Обновление возможных контактов. Обновляет таблицу известных пользователей и содержимое предполагаемых контактов
    def update_possible_contacts(self):
        try:
            self.s.users_lst_update()
        except OSError:
            pass
        else:
